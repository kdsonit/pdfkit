...
var SVGPath = (function() {
  ...
  let fixRoundingError = undefined;
  SVGPath = class SVGPath {
    static initClass() {
      ...
      segmentToBezier = function(cx, cy, th0, th1, rx, ry, sin_th, cos_th) {
        ...
        const x1 = fixRoundingError((cx + Math.cos(th0)) - (t * Math.sin(th0)));
        const y1 = fixRoundingError(cy + Math.sin(th0) + (t * Math.cos(th0)));
        const x3 = fixRoundingError(cx + Math.cos(th1));
        const y3 = fixRoundingError(cy + Math.sin(th1));
        const x2 = fixRoundingError(x3 + (t * Math.sin(th1)));
        const y2 = fixRoundingError(y3 - (t * Math.cos(th1)));
