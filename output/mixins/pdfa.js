...
module.exports = {
  ...
  //
  pdfaXMP() {
    let s =  '<?xpacket begin="\xEF\xBB\xBF" id="W5M0MpCehiHzreSzNTczkc9d"?>\n'; // "begin=" contains UTF-8 BOM (U+FEFF)
    ...
    if (this.info.Keywords) { s += `        <pdf:Keywords>${this.info.Keywords}</pdf:Keywords>\n`; }
    s += '      </rdf:Description>\n';
    ...
    if (this.info.Title) { s += `        <dc:title><rdf:Alt><rdf:li xml:lang="x-default">${this.info.Title}</rdf:li></rdf:Alt></dc:title>\n`; }
    if (this.info.Subject) { s += `        <dc:subject><rdf:Bag><rdf:li>${this.info.Subject}</rdf:li></rdf:Bag></dc:subject>\n`; }
    if (this.info.Author) { s += `        <dc:creator><rdf:Seq><rdf:li>${this.info.Author}</rdf:li></rdf:Seq></dc:creator>\n`; }
    s += '      </rdf:Description>\n';
    ...
    if (this.options.pdfaAdditionalXmpRdf) { s += this.addRdfAbout(this.options.pdfaAdditionalXmpRdf) + "\n"; } // optionally given rdf
